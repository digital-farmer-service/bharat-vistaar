...
import { Send, Mic, MicOff, ChevronUp, ChevronLeft, ChevronRight, Info, MicVocal } from "lucide-react";
...
export function ChatInterface() {
  ...
  const [suggestionRefreshInterval, setSuggestionRefreshInterval] = useState(10000); // 10 seconds in milliseconds
    ...
    if (maxSuggestionRefreshes > 1) {
      suggestionRefreshTimerRef.current = setInterval(() => {
        setSuggestionRefreshCount(prevCount => {
          ...
          if (newCount >= maxSuggestionRefreshes && suggestionRefreshTimerRef.current) {
            clearInterval(suggestionRefreshTimerRef.current);
  ...
  const handleSendFromVoiceDialog = async (voiceText: string): Promise<{ botMessageId: string; responseText: string }> => {
    const userMessageId = addMessage(voiceText, true);
    const loadingMessageId = addMessage("", false, { isLoading: true });
    setIsMessageLoading(true);
    try {
      const result = await sendMessageToApi(voiceText, loadingMessageId);
      return { botMessageId: result.messageId, responseText: result.finalText || "" };
    } finally {
      setIsMessageLoading(false);
    }
  };
        ...
        >
          <div className="p-3 relative">
            ...
            <div className="text-xs text-muted-foreground text-center mt-1 flex items-center justify-center">
              ...
              {(t("disclaimerText") as string) || "Bharat Vistaar is AI and can make mistakes. Please verify sources."}
            </div>
          </div>
        </div>
      </div>
