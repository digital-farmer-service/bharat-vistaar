...
import { retryWithBackoff, DEFAULT_RETRY_CONFIG, isRetryableError, type RetryConfig } from './retry-utils';
...
class ApiService {
  ...
  async getSuggestions(session: string, targetLang: string = 'hi'): Promise<SuggestionItem[]> {
  ...
  async streamTranscript(
    ...
    if (!reader) {
      ...
      const base64 = (json?.audio_data || json?.data?.audio_data || '') as string;
    ...
    const flushDecodable = () => {
      ...
      try {
        ...
        console.warn('Base64 decode failed for slice; skipping until next chunk');
      }
    };
  ...
  async fetchAuthToken(metadata: string): Promise<string> {
    try {
      ...
      const response = await axios.post<AuthResponse>(`${this.apiUrl}/api/token`, {
        metadata
      }, {
        headers: {
          'Content-Type': 'application/json'
