...
!(function (s) {
  function p(a, k, b, h, l, j, m) {
    return (
      (a = a + ((k & b) | (~k & h)) + l + m), ((a << j) | (a >>> (32 - j))) + k
    );
  }
  function m(a, k, b, h, l, j, m) {
    return (
      (a = a + ((k & h) | (b & ~h)) + l + m), ((a << j) | (a >>> (32 - j))) + k
    );
  }
    ...
    return (
      ...
      (this.telemetry.impression = function (pageid, type, subtype, data) {
        if (void 0 == pageid || void 0 == type)
          return void console.error("Invalid impression data");
        var eksData = {
          stageid: pageid,
      ...
      (this.telemetry.interact = function (data) {
        if (!instance.hasRequiredData(data, ["type", "id"]))
          return void console.error("Invalid interact data");
        var eksData = {
      ...
      (this.telemetry.endAssessment = function (assessStartEvent, data) {
        ...
        var endeks = Object.assign(assessStartEvent.edata.eks, {
          ...
          mc: data.mc,
          length: Math.round(
            (new Date().getTime() - assessStartEvent.ets) / 1e3
          ),
        });
      ...
      (this.telemetry.interrupt = function (data) {
        if (!instance.hasRequiredData(data, ["type"]))
          return void console.error("Invalid interrupt data");
      ...
      (this.telemetry.error = function (data) {
        if (!instance.hasRequiredData(data, ["err", "errtype"]))
          return void console.error("Invalid error data");
        var eksData = {
          err: data.err,
          type: data.errtype,
          env: data.env || "",
      ...
      (this.telemetry.exdata = function (type, data) {
        instance.getEvent("OE_XAPI", { xapi: data }),
      ...
      (instance.init = function (config, contentId, contentVer, type) {
        if (Telemetry.initialized)
          return console.log("Telemetry is already initialized.."), !1;
        ...
        return instance.hasRequiredData(requiredData, [
          ...
          ? ((_defaultValue.gdata = { id: contentId, ver: contentVer }),
            ...
            void 0 != Telemetry.config.pdata.pid &&
      ...
      (instance.getEvent = function (eventId, data) {
        ...
        console.log(
          ...
          eventId,
          data?.qid,
          window.__RESPONSE_TIMERS__?.[data?.qid]
        );
        const ctx = window.__FINGERPRINT_CONTEXT__;

        if (ctx?.ready && ctx.data) {
          ctx.data.device_lifecycle.last_seen_at = eventObj.ets;

          if (eventId === "OE_END") {
            ctx.data.session.session_end_at = eventObj.ets;
          }

          // 1️⃣ Assign fingerprint_details FIRST
          eventObj.edata.eks.fingerprint_details = ctx.data;

          // 2️⃣ Then safely attach performance
          if (eventId === "OE_ITEM_RESPONSE" && data.qid) {
            const timer = window.__RESPONSE_TIMERS__?.[data.qid];

            // if (!timer || !timer.paintTime || !timer.startTime) return eventObj;

            // if (timer?.responseStart && timer?.paintTime) {
            // if (timer) {
            ...
            //     timer.responseEnd && timer.paintTime
            //       ? timer.paintTime - timer.responseEnd
            //       : undefined;

            //   eventObj.edata.eks.fingerprint_details.performance = {
            //     server_response_time_ms: serverResponseTime
            //       ? Math.round(serverResponseTime)
            //       : undefined,
            //     browser_render_time_ms: Math.round(browserRenderTime),
            //     total_response_time_ms: Math.round(
            //       timer.paintTime - timer.startTime
            //     ),
            //   };
            // }
            ...
            if (timer) {
              const serverResponseTime =
                timer.networkStartTime && timer.networkEndTime
                  ? timer.networkEndTime - timer.networkStartTime
                  : undefined;

              const browserRenderTime =
                timer.paintTime && (timer.renderStart || timer.startTime)
                  ? timer.paintTime - (timer.renderStart || timer.startTime)
                  : undefined;
      ...
      (instance.addEvent = function (telemetryEvent) {
        if (Telemetry.initialized) {
          telemetryEvent.mid =
            "OE_" + CryptoJS.MD5(JSON.stringify(telemetryEvent)).toString();
          ...
          console.log(
            "Telemetry is not initialized. Please start Telemetry to log events."
          );
      }),
  ...
  TelemetrySyncManager = {
    ...
    sendTelemetry: function (event) {
      ...
      if (
        ...
        "OE_END" == telemetryEvent.eid.toUpperCase() ||
